...
typedef vector<tuple<std::string, int, int, int, int, int>> MemoryLayoutType;
...
class CodeContainer : public virtual Garbageable {
   protected:
    ...
    list<std::string> fUIMacroActives;
    list<std::string> fUIMacroPassives;

    void merge(set<std::string>& dst, set<std::string>& src)
    {
   ...
   public:
    ...
    DeclareFunInst* generateGetIORate(const std::string& name, const std::string& obj, vector<int>& io, bool ismethod,
                                      bool isvirtual);
    ...
    ValueInst* pushFunction(const std::string& name, Typed::VarType result, vector<Typed::VarType>& types,
                            const Values& args);
