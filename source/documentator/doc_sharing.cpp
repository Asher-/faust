...
void DocCompiler::sharingAnnotation(const Priority& priority, Tree sig)
{
    ...
    if (count > 0) {
        ...
        const Priority& priority_visitor = getCertifiedSigType(sig)->priority();

        // check "time sharing" cases
        if (priority_visitor < priority) {
